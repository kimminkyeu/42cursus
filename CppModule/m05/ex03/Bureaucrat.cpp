...
// NOTE:  ex01
Bureaucrat& Bureaucrat::signForm(Form& form) const
{
	try
	{
		form.beSigned(*this);
		// here, if success, try will run below code. ( WARN:  this could lead to memory leak!)
	...
	*/
	return (const_cast<Bureaucrat&>(*this));
}
...
void Bureaucrat::executeForm(const Form& form) const
{
	...
	{
		std::cout << *this << " can't execute form because " << e.what() << "\n";
	}
	/**
	 * ? To handle each exception case....
	catch (...)  --> 위에서 안걸린 모든 exception은 여기로.
