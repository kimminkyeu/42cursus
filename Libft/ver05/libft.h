...
/*   Updated: 2022/03/15 23:44:30 by minkyeki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Copy up to dstsize - 1 characters from the NUL-terminated string src to dest, NUL-terminating the result. */
...
/* Fill the first len bytes of the memory area pointed to by b with the constant byte c. */
 ...
 * If the multiplication of count and size would result in integer overflow, then ft_calloc() returns an error.*/
...
/* Allocates and returns a string resulting from successive applications of ’f’ to each character of the string ’s’.*/
extern char		*ft_strmapi(char const *s, char (*f)(unsigned int, char));

/* Applies the function 'f' to each character of the string passed as argument, and passing its index to 'f' as first argument. */
 ...
 * (1) t_list variable ’content’ is initialized with the value of the parameter ’content’.
...
/* Delete one element pointed by 'lst' using the function 'del', then free the element. */
extern void		ft_lstdelone(t_list *lst, void (*del)(void *));

/* Delete content (using 'del') and free every element of the list, then set the pointer to the list to NULL. */
