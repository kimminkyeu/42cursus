...
/*   Updated: 2022/04/28 14:20:51 by minkyeki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vector4	*create_data_vec4_malloc(int _x, int _y, int _z, int _color)
{
	t_vector4	*pa_result;
...
static void	mp_convert_and_push(char **strs, t_darray *pa_map, int x, int y)
{
	...
	{
		data = create_data_vec4_malloc(x, y, z, ft_atoi_base(str + 5, "0123456789abcdef"));
		/* FIXME : handle sigabort --> null guard is important  */
		if (pa_map != NULL && data != NULL)
			darray_push_back(pa_map, data);
	}
	else if (ft_strlen(str) > 1)
	{
		data = create_data_vec4_malloc(x, y, z, ft_atoi_base(str + 4, "0123456789abcdef"));
		/* FIXME : handle sigabort --> null guard is important  */
		if (pa_map != NULL && data != NULL)
			darray_push_back(pa_map, data);
	}
	...
	{
		data = create_data_vec4_malloc(x, y, z, DEFAULT_PT_COLOR);
		/* FIXME : handle sigabort --> null guard is important  */
		if (pa_map != NULL && data != NULL)
			darray_push_back(pa_map, data);
	}
