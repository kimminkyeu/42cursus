...
/*   Updated: 2022/05/03 21:24:12 by minkyeki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_key_change_projection(int key_code, t_mlx *fdf)
{
	if (key_code == KEY_1)
	{
		ft_putstr_fd("projection none\n", STDOUT);
		fdf->camera.projection_matrix = project_nothing();
	}
	else if (key_code == KEY_2)
	{
		ft_putstr_fd("orthographic_projection\n", STDOUT);
		fdf->camera.projection_matrix = orthographic_projection();
	}
	engine_render(fdf);
}
