...
t_mat4x4	perspective_projection(t_mlx *fdf)
{
	static t_mat4x4 mat;

	float	fFov_cotangent = 1.0f / tanf(get_radian(fdf->camera.fFov / 2.0f));

	mat.m[0][0] = fFov_cotangent;
	mat.m[1][1] = fFov_cotangent;
	mat.m[2][2] = (fdf->camera.fFar + fdf->camera.fNear) / (fdf->camera.fFar - fdf->camera.fNear);
	mat.m[2][3] = (2.0f * fdf->camera.fFar * fdf->camera.fNear) / (fdf->camera.fFar - fdf->camera.fNear);
	mat.m[3][2] = -1.0f;
	mat.m[3][3] = 0.0f;
...
t_mat4x4	isometric_projection(void)
{
	static  t_mat4x4 mat;
...
{
	fdf->camera.projection_matrix = isometric_projection();
	fdf->camera.angle.x = 0.0f;
	fdf->camera.zoom_rate = 1.0f;
	...
	fdf->camera.fFov = 90.0f;
	fdf->camera.fFar = 1000.0f;
	fdf->camera.fNear = 0.1f;
}
