...
/*   Updated: 2022/05/15 03:11:31 by minkyeki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	run_process_pid(t_cmd cmd_info, int *pipe_in, int *pipe_out)
{
	ft_printf("\ncmd: %s\n", cmd_info.cmd);
	ft_printf("pipe_in --> read:%d\n", pipe_in[READ_END]);
	ft_printf("pipe_out --> write:%d\n", pipe_out[WRITE_END]);
